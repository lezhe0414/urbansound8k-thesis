...
15. 已建立獨立 post-formal exploratory branch，準備只在 development folds 1、4、7 比較 MN20 三 seed probability ensemble、validation top-3 checkpoint weight averaging，以及 loss-only focal-loss replacement。此研究不讀取、不重跑，也不以既有 formal test folds 選模。
...
5. Post-formal MN20 延伸只作 development-only 探索，結果不得覆寫已鎖定的正式 10-fold 結論。

## 目前完成度判斷

專案基礎設施、資料處理、模型 pipeline、CNN 受控 augmentation 搜尋、EMA、3-seed ensemble，以及 EfficientAT v1/v2/recommended transfer-learning studies 均已完成。唯一 MN20 方法已依 development folds 鎖定，且正式 10-fold 已完成。整體目標尚未完成，因為 from-scratch 主模型的固定 10-fold、公平三模型比較、最終圖表解讀與 8 頁論文仍需完成。
