...
| EfficientAT pretrained CNN | `src/models/pretrained_efficientat.py`、`src/vendor/efficientat/` | AudioSet-pretrained compact CNN wrapper 與官方 frontend | 進行中 | unit tests + Colab development run | 上游 commit `a425fdc`；fold 10 封存 |
| Pretrained CNN waveform pipeline | `src/data/urbansound8k_waveform.py`、`scripts/cache_urbansound8k_waveforms.py` | 32 kHz、5 秒 waveform 載入與獨立 cache | 進行中 | unit tests + cache manifest | 不覆寫 raw 或既有 Mel cache |
| Pretrained CNN development runner | `src/train_pretrained_cnn.py`、`scripts/run_pretrained_cnn_transfer.py` | folds 1、4、7 linear probe / partial fine-tuning | 進行中 | tests + Colab A100 | 只依 validation Macro F1 mean/std |
...
| AudioSet-pretrained CNN transfer study | `docs/experiments/pretrained-cnn-transfer.md` | 第三模型的 protocol、設定、結果與公平比較 | 進行中 | folds 1、4、7 validation outputs | Linear probing 待執行，fold 10 未評估 |
