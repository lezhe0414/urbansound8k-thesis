...
| EfficientAT pretrained CNN | `src/models/pretrained_efficientat.py`、`src/vendor/efficientat/` | AudioSet-pretrained compact CNN wrapper 與官方 frontend | 已確認 | unit tests + Colab A100 runs | 上游 commit `a425fdc`；唯一設定鎖定後才評估 fold 10 |
| Pretrained CNN waveform pipeline | `src/data/urbansound8k_waveform.py`、`scripts/cache_urbansound8k_waveforms.py` | 32 kHz、5 秒 waveform 載入與獨立 cache | 已確認 | unit tests + cache manifest | 8732 筆；不覆寫 raw 或既有 Mel cache |
| Pretrained CNN development runner | `src/train_pretrained_cnn.py`、`scripts/run_pretrained_cnn_transfer.py` | folds 1、4、7 linear probe / partial fine-tuning | 已確認 | tests + Colab A100 | 支援 gradual unfreezing、post-frontend Mixup 與可設定解凍深度；只依 validation Macro F1 mean/std |
...
| EfficientAT recommended study | `docs/experiments/pretrained-cnn-recommended-study.md`、`configs/pretrained_cnn_mn20_locked_last2.yaml` | Waveform augmentation、TTA、固定三 seed ensemble、MN20 與正式 10-fold protocol | 已完成 | 41 tests + development folds 1/4/7 + locked formal 10-fold | Dev F1 `0.89069 ± 0.01165`；formal F1 `0.87686 ± 0.04048`；Drive `pretrained_cnn_recommended/` |
| EfficientAT post-formal exploration | `docs/experiments/pretrained-cnn-postformal-exploration.md` | MN20 三 seed、checkpoint averaging 與 focal-loss development-only 比較 | 已完成 | 27 manifests + folds 1/4/7 validation only | Focal + ensemble F1 `0.89395 ± 0.00932`；checkpoint averaging 無效；Drive `pretrained_cnn_postformal/`；未評估 test |
| EfficientAT bold breakthrough study | `docs/experiments/pretrained-cnn-bold-breakthrough.md`、`configs/pretrained_cnn_bold_*.yaml`、`scripts/run_pretrained_cnn_bold_study.py`、`scripts/run_pretrained_cnn_bold_multiseed_cross_scale.py` | MN30/MN40、BatchNorm freeze 與 cross-scale ensemble 的 post-formal development-only 比較 | 已完成 | folds 1/4/7 validation only；第二階段 12 manifests | MN20+MN40 seed-42 F1 `0.90128 ± 0.00982`；六模型 multi-seed F1 `0.90104 ± 0.00920`；Drive `pretrained_cnn_bold_multiseed_cross_scale_6237825_20260817_141117/`；未評估 test |
| EfficientAT nested stacking study | `docs/experiments/pretrained-cnn-stacking-breakthrough.md`、`src/stacking.py`、`scripts/run_pretrained_cnn_stacking_study.py` | 六模型 nested leave-one-development-fold-out logistic stacking | 已完成；未採用 | 4 stacking tests + 22 transfer regression tests + folds 1/4/7 validation only | Stacking F1 `0.87438 ± 0.01625` vs equal average `0.90104 ± 0.00920`；Drive `pretrained_cnn_nested_stacking_c1e734c_20260817_150141/`；未評估 test |
