...
| 2026-08-17 | 操作 Colab A100 完成 MN20 post-formal development-only 探索，執行 27 個 runs，比較 weighted CE、class-balanced focal loss、validation top-3 checkpoint averaging 與固定三 seed probability ensemble，並核對全部 manifests 及 Drive 備份 | `docs/experiments/pretrained-cnn-postformal-exploration.md`；Drive `pretrained_cnn_postformal/` | 將 focal + ensemble 的小幅改善只列為探索性證據 | 勝出 F1 `0.89395 ± 0.00932`；所有 runs 均未評估 test；checkpoint averaging 退步 |
| 2026-08-17 | 預註冊並實作 EfficientAT bold breakthrough study，加入官方 MN30/MN40、partial fine-tuning BatchNorm statistics freeze、跨尺度 probability ensemble、固定門檻的三 seed 擴展與封存 fold 10 的 runner | `src/models/pretrained_efficientat.py`、`src/evaluate_pretrained_cnn.py`、`configs/pretrained_cnn_bold_*.yaml`、`scripts/run_pretrained_cnn_bold_study.py`、`docs/experiments/pretrained-cnn-bold-breakthrough.md` | 待 Colab development-only 結果 | 正式 10-fold 不參與選模；不修改 raw data 或 cache |
| 2026-08-17 | 操作 Colab A100 完成 EfficientAT bold breakthrough development-only study，比較 MN20/MN30/MN40、BatchNorm freeze、七組跨尺度 ensemble，並依預註冊門檻補跑 MN40 三 seeds | `docs/experiments/pretrained-cnn-bold-breakthrough.md`；Drive `pretrained_cnn_bold_breakthrough_b6848c1_20260817_130915/` | 使用者需確認是否將探索性結果納入論文補充 | MN20+MN40 fixed-seed F1 `0.90128`，但 MN40 3-seed 只有 `0.89440`；33 manifests 均未評估 test |
| 2026-08-17 | 預註冊完整 MN20 + MN40 cross-scale multi-seed 穩健性測試，建立可重用既有 checkpoints、補訓缺少 seeds、評估三個 same-seed pairs 與唯一六模型集成的 development-only runner | `scripts/run_pretrained_cnn_bold_multiseed_cross_scale.py`、`docs/experiments/pretrained-cnn-bold-breakthrough.md` | 待 Colab folds 1/4/7 validation 執行 | 成功門檻執行前固定；fold 10 無執行入口；不修改 raw data 或 cache |
| 2026-08-17 | 操作 Colab A100 完成 MN20 + MN40 cross-scale multi-seed 穩健性測試，補訓 MN20 seeds 123/2026、評估三個 same-seed pairs 與唯一六模型 probability ensemble，並核對 Drive 備份及 fold 10 封存 | `scripts/run_pretrained_cnn_bold_multiseed_cross_scale.py`、`docs/experiments/pretrained-cnn-bold-breakthrough.md`；Drive `pretrained_cnn_bold_multiseed_cross_scale_6237825_20260817_141117/` | 將結果只列為 post-formal exploratory evidence，不追加 fold 10 | 六模型 F1 `0.90104 ± 0.00920`；較歷史 MN20 三 seed高 `0.00709`，較固定-seed screen低 `0.00024`；12 manifests 均未評估 test |
| 2026-08-17 | 預註冊、實作並操作 Colab A100 完成六模型 nested leave-one-development-fold-out logistic stacking，包含 training-only standardisation、inner regularisation selection、outer evaluation、測試與 Drive 備份 | `src/stacking.py`、`scripts/run_pretrained_cnn_stacking_study.py`、`tests/test_pretrained_cnn_stacking.py`、`docs/experiments/pretrained-cnn-stacking-breakthrough.md`；Drive `pretrained_cnn_nested_stacking_c1e734c_20260817_150141/` | 拒絕 stacking，保留六模型等權平均作探索性主結果 | Stacking F1 `0.87438 ± 0.01625`，低於 equal average `0.90104 ± 0.00920`；fold 10 封存且未評估 test |
